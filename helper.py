...
def perform_kmeans(final_embedding, labels, n_clusters):
    ...
    for seed in seeds:
        ...
        silhouette_ = silhouette_score(final_embedding, y_pred, metric="euclidean")
    ...
    return acc_mean, acc_std, nmi_mean, nmi_std, silhouette_mean, silhouette_std

def perform_knn(final_embedding, labels, n_clusters):
    seeds = [0,12,21,42,1234]
    acc = []
    for seed in seeds:
        X_train, X_test, y_train, y_test = train_test_split(final_embedding, labels, test_size=0.25, random_state=seed)
...
def get_sigma_params(sigmas_init,disease):
    if disease == "brca":
        sig_quant = 0.1
        sig_df = 10
        sig_scale = (max(np.quantile(sigmas_init, q=0.10), 1e-2) ** 2) * chi2.ppf(1 - sig_quant, sig_df) / sig_df
    elif disease == "kirc":
        sig_quant = 0.9
        sig_df = 3
        sig_scale = (max(np.quantile(sigmas_init, q=0.05), 1e-3) ** 2) * chi2.ppf(1 - sig_quant, sig_df) / sig_df
    else:
